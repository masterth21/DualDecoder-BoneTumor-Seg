...
def prepare_mask(path: str, resize: dict, normalize_mask: dict):
    ...
    read mask --> resize --> map class index [0: Không u, 1: U lành, 2: U ác] --> return as int32
    """
    ...
    # Chuyển đổi nhãn pixel thành class index:
    # 0 -> 0: Không u (Nền)
    # 128 -> 1: U lành (Benign)
    # 255 -> 2: U ác (Malignant)
    if np.any(mask > 2):
        mapped_mask = np.zeros_like(mask, dtype=np.int32)
        mapped_mask[(mask > 64) & (mask <= 192)] = 1
        mapped_mask[mask > 192] = 2
        mask = mapped_mask
    elif normalize_mask.VALUE:
